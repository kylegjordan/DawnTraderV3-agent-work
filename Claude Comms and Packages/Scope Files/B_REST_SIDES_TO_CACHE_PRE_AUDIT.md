...
| **P3** | **The phantom counter (C3's instrument):** in each of the three REST ticker sites, a write whose key matches no requested symbol increments a per-pass count. `refreshBucket` prints one line per pass: symbols written, phantoms (with their keys), and the `sidesWriter` census across the bucket. | §2.2, C3 |
...
| **P4b** | **Pending Q-A:** remove the dead pairs from the four hardcoded lists in §2.5, after reading each list's consumer at Step 3. | §2.5 |
